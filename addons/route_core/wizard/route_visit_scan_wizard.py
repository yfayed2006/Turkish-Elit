...
class RouteVisitScanWizard(models.TransientModel):
    ...
    active_lot_status = fields.Selection([("normal", "Normal"), ("near_expiry", "Near Expiry"), ("expired", "Expired")], string="Lot Status", compute="_compute_active_lot_state", store=False)
    ...
    def _onchange_barcode_preview(self):
        for rec in self:
            ...
            if scan_info.get("scan_type") == "box":
                ...
                rec.detected_packaging_name = scan_info.get("packaging_display_name") or rec.visit_id._get_packaging_display_name(scan_info.get("packaging"), product=product)
                rec.quantity = 1.0
