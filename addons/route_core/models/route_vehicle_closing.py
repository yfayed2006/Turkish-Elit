...
class RouteVehicleClosing(models.Model):
    ...
    def action_close_day(self):
        for rec in self:
            ...
            if not rec.count_started_datetime:
                raise UserError(_("Please start and complete the vehicle count before closing the day."))
    ...
    def action_reset_to_draft(self):
        self.write({"state": "draft", "close_datetime": False})
        return True
    ...
    def action_open_vehicle_stock_snapshot(self):
        ...
        return {
            ...
            "view_mode": "list,form",
            "views": [(self.env.ref("route_core.view_route_vehicle_stock_snapshot_list").id, "list"), (False, "form")],
            "search_view_id": self.env.ref("route_core.view_route_vehicle_stock_snapshot_search").id,
            "target": "current",
            "domain": [("location_id", "child_of", vehicle_location.id), ("quantity", ">", 0)],
            "context": {"search_default_filter_positive_qty": 1, "default_location_id": vehicle_location.id},
        }
...
class RouteVehicleClosingLine(models.Model):
    ...
    lot_id = fields.Many2one("stock.lot", string="Lot/Serial Number", ondelete="restrict")
    expiry_date = fields.Datetime(string="Expiry Date", related="lot_id.expiration_date", readonly=True)
    alert_date = fields.Datetime(string="Alert Date", related="lot_id.alert_date", readonly=True)
    ...
    variance_qty = fields.Float(string="Variance Qty", compute="_compute_variance", digits="Product Unit of Measure")
    variance_status = fields.Selection([("match", "Match"), ("short", "Short"), ("over", "Over")], string="Variance Status", compute="_compute_variance")
...
class RoutePlan(models.Model):
    ...
    vehicle_closing_count = fields.Integer(string="Vehicle Closings", compute="_compute_vehicle_closing_stats")
    vehicle_closing_state = fields.Selection([("draft", "Draft"), ("closed", "Closed"), ("cancelled", "Cancelled")], string="Vehicle Closing Status", compute="_compute_vehicle_closing_stats")
