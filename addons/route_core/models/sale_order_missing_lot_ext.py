...
class SaleOrder(models.Model):
    ...
    route_available_product_ids = fields.Many2many(
        "product.product",
        string="Available Vehicle Products",
        compute="_compute_route_available_order_products",
        store=False,
    )
    route_available_product_tmpl_ids = fields.Many2many(
        "product.template",
    ...
    def _compute_route_available_order_products(self):
        Quant = self.env["stock.quant"]
        Product = self.env["product.product"]
        ProductTemplate = self.env["product.template"]
        product_cache = {}
        ...
        source_locations = {
            order.route_source_location_id.id
            for order in self
        ...
        for location_id in source_locations:
            ...
            for quant in quants:
                ...
                if not product:
                    continue
                available_by_product[product.id] = available_by_product.get(product.id, 0.0) + SaleOrderLine._route_get_quant_available_qty(quant)
            product_ids = [product_id for product_id, qty in available_by_product.items() if qty > 0]
            products = Product.browse(product_ids)
            product_cache[location_id] = products
            template_cache[location_id] = products.mapped("product_tmpl_id")
        ...
        for order in self:
            if order.route_order_mode == "direct_sale" and order.route_source_location_id:
                ...
                order.route_available_product_ids = Product
                order.route_available_product_tmpl_ids = ProductTemplate
