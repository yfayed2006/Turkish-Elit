...
class RouteWeeklySchedule(models.Model):
    ...
    def _compute_schedule_stats(self):
        for rec in self:
            ...
            rec.off_day_stop_count = (
                len(rec.line_ids.filtered(lambda line: line.weekday == rec.off_day))
                if rec.off_day
                else 0
            )
    ...
    def _cleanup_duplicate_lines(self):
        for rec in self:
            ...
            for line in rec.line_ids.sorted(key=lambda item: (item.weekday or "", item.sequence, item.id)):
                key = (line.weekday or "monday", line.outlet_id.id)
    ...
    def create(self, vals_list):
        ...
        for schedule, vals in zip(schedules, vals_list):
            ...
            schedule._cleanup_duplicate_lines()
    ...
    def _build_schedule_name(self):
        self.ensure_one()
        date_token = fields.Date.to_string(
            self.week_start_date or fields.Date.context_today(self)
        ).replace("-", "")
    ...
    def _load_template_lines(self):
        ...
        commands += [fields.Command.create(values) for values in self._sanitize_line_dicts(line_dicts)]
    ...
    def action_generate_daily_plans(self):
        ...
        for rec in self:
            ...
            processable_lines = (
                rec.line_ids.filtered(lambda line: line.weekday != rec.off_day)
                if rec.off_day
                else rec.line_ids
            )
            if not processable_lines:
                raise UserError(
                    _(
                        "All scheduled stops fall on the configured weekly off day. Move them to working days before generating daily plans."
                    )
                )
    ...
    def action_open_day_stop_wizard(self):
        ...
        weekday = self.env.context.get("default_weekday") or "monday"
...
class RouteWeeklyScheduleLine(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if outlet_id and not vals.get("area_id"):
                vals["area_id"] = self.env["route.outlet"].browse(outlet_id).area_id.id
    ...
    def write(self, vals):
        ...
        if vals.get("outlet_id") and not vals.get("area_id"):
            vals["area_id"] = self.env["route.outlet"].browse(vals["outlet_id"]).area_id.id
