...
class RouteVisit(models.Model):
    ...
    def _compute_ux_workflow(self):
        for rec in self:
            rec.ux_can_scan_barcode = rec.state == "in_progress" and rec.visit_process_state in ("checked_in", "counting")
            rec.ux_can_scan_returns = rec.state == "in_progress" and rec.visit_process_state == "counting" and not rec.returns_step_done
            ...
            rec.ux_can_generate_refill = rec.state == "in_progress" and rec.visit_process_state == "reconciled" and not (rec.refill_datetime or rec.has_refill or rec.no_refill)
            rec.ux_can_open_pending_refill = bool(rec.has_pending_refill or rec.refill_backorder_id)
            rec.ux_can_collect_payment = rec.state == "in_progress" and rec.visit_process_state == "reconciled" and bool(rec.refill_datetime or rec.has_refill or rec.no_refill)
            rec.ux_can_confirm_payments = rec.state == "in_progress" and rec.visit_process_state == "reconciled" and bool(rec.payment_ids.filtered(lambda p: p.state == "draft"))
            rec.ux_can_skip_collection = rec.ux_can_collect_payment
            rec.ux_can_open_payments = rec.ux_can_collect_payment
            rec.ux_can_finish_visit = rec.state == "in_progress" and rec.visit_process_state in ("collection_done", "ready_to_close")
            ...
            if rec.state == "draft":
                ...
                rec.ux_stage_title = "Scan returns"
                rec.ux_stage_help = "Record returns or choose No Returns."
            ...
            elif rec.visit_process_state == "reconciled" and not (rec.refill_datetime or rec.has_refill or rec.no_refill):
                rec.ux_stage = "refill"
            ...
            elif rec.visit_process_state == "reconciled" and not rec.payment_ids.filtered(lambda p: p.state == "confirmed") and not rec.collection_skip_reason:
    ...
    def action_ux_returns_step(self):
        self.ensure_one()
        return self.action_ux_returns_step()

    def action_ux_no_returns(self):
        self.ensure_one()
        return self.action_ux_no_returns()
